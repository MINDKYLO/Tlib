...
@app.route('/admin/import-equipment', methods=['GET', 'POST'])
@admin_required
def import_equipment():
    if request.method == 'POST':
        f = request.files.get('excel_file')
        if not f or not f.filename.endswith(('.xlsx', '.xls')):
            flash('กรุณาอัพโหลดไฟล์ Excel (.xlsx) เท่านั้น', 'danger')
        ...
        try:
            ...
            for i, row in enumerate(rows, start=2):
                ...
                category = str(row[1]).strip() if len(row) > 1 and row[1] else 'Other'
                serial   = str(row[2]).strip() if len(row) > 2 and row[2] else None
                ...
                try:
                    db_execute(conn, '''INSERT INTO equipment
                               (name, category, serial_number, brand, model, description)
                               VALUES (%s,%s,%s,%s,%s,%s)''',
                    ...
                    conn.rollback()
                    skipped.append(f'แถว {i}: Serial "{serial}" ซ้ำ')

            conn.close()
            if added:
                flash(f'นำเข้าสำเร็จ {added} รายการ', 'success')
            if skipped:
                flash('ข้ามรายการซ้ำ: ' + ', '.join(skipped), 'warning')
            return redirect(url_for('equipment_list'))
...
@app.route('/admin/import-equipment/template')
@admin_required
def download_import_template():
    ...
    headers = ['ชื่ออุปกรณ์ *', 'หมวดหมู่ *', 'Serial Number', 'ยี่ห้อ', 'รุ่น', 'รายละเอียด']
    hdr_fill = PatternFill('solid', fgColor='2563EB')
    hdr_font = Font(bold=True, color='FFFFFF', size=11)
    ...
    # ตัวอย่างข้อมูล
    examples = [
        ('MacBook Pro 14"', 'Notebook', 'NB-001', 'Apple', 'MacBook Pro M3', '16GB RAM 512GB SSD'),
        ('Dell Monitor 27"', 'Monitor', 'MN-001', 'Dell', 'U2722D', '4K IPS'),
        ('iPhone 15 Pro', 'Mobile', 'MB-001', 'Apple', 'iPhone 15 Pro', '256GB'),
    ]
    for r, ex in enumerate(examples, 2):
        for c, v in enumerate(ex, 1):
            ws.cell(row=r, column=c, value=v)

    widths = [25, 15, 18, 12, 20, 30]
    for i, w in enumerate(widths, 1):
        ws.column_dimensions[get_column_letter(i)].width = w
